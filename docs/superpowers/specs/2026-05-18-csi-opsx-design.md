...
csi-opsx/
  ...
  src/
    ...
    commands/
      explore/
        SKILL.md            ← /csi-opsx:explore skill template (asset, not compiled)
      propose/
        SKILL.md            ← /csi-opsx:propose skill template (asset, not compiled)
        agents.ts           ← ProposerAgent + ReviewerAgent configs
        harness.ts          ← proposer→reviewer loop orchestration
      apply/
        SKILL.md            ← /csi-opsx:apply skill template (asset, not compiled)
      archive/
        SKILL.md            ← /csi-opsx:archive skill template (asset, not compiled)
    lib/
...
Skill file content is the same across all agents — agent-neutral markdown. Command file format and path are agent-specific and handled by the adapter in `lib/adapters/`.
